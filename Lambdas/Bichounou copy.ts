...
ExpressionAttributeNames: {'new_key' : 'bichounouid'}, 
...
try{
    ...
    const response = await bdd.update(paramsUpdate, function(err, data) {
    }).promise(); 
    const data = await bdd.scan(params).promise();
    
    return { statusCode : 200 , body: "Retour de la base " + JSON.stringify(data.Items)}
